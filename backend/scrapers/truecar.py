...
class TrueCarScraper(BaseScraper):
    """Scraper for TrueCar.com listings"""
    
    def can_handle(self, url: str) -> bool:
        """Check if URL is from TrueCar"""
        parsed = urlparse(url)
        return 'truecar.com' in parsed.netloc.lower() and ('listing' in parsed.path.lower() or 'used-cars-for-sale' in parsed.path.lower())
    
    def scrape(self, url: str) -> Dict[str, Any]:
        """Scrape car data from TrueCar"""
        try:
            ...
            # TrueCar often has JSON-LD structured data
            ...
            car_data.setdefault('platform', 'TrueCar')
            ...
            logger.error(f"Error scraping TrueCar URL {url}: {e}")
            raise
